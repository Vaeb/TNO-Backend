...
const noFormer = (reg: RegExp) => mergeRegex([/(?<!(?:\b|_)(?:vs?|versus|against|e?x|former|fake|wanna\s*be|fighting|clapping|with|becom\w+\s+a|for|then)(?:\s+(?:th?e|some|all|every|el|la|da))?[^a-z0-9]*|vs?)/i, '(?:', reg, ')']);
...
export const npFactionsRegex = {
    ...
    doj: noLater(noFormer(/\bdoj\b|\bspu\b|department\s*of\s*justice|judge\b(?![\s]*me)|\bd\.a\b|\ba\.?d\.?a\b|(?<!\b(?:former|ex|aspiring)[\s\-]*)(?:lawyer|attorney)|para[\s\-]*legal/i)),
    ...
    police: noLater(noFormer(
        mergeRegex([
            ...
            /(?:\bcop\b|chief of police|\b[56](?:\d[1-9]|[1-9]\d)\b|officer|\bfinal\s*eval|\bfto|der?puty(?![\s\-_]*(?:mayor|warden))|\bd-\d|\bdispatch\b|ride[\s\-_.]*along|sergeant|lieutenant|corporal|sheriff|\btrooper|\bcadet|\bcaderp|\b(?:ranger|dt|sgt|cpl|cpt|ofc|lspd|sasp|bcso|pbso|sdso|police[\s\-_]*academy|lt(?![^|!]*\bjones\b))\b)/i,
            /(?!\?|[\s\-_]+copium)/i,
        ])
    )),
    medical: noLater(/(?<!then\b.*|!)(?:doctor|\b[teplch]-\d{1,2}\b|paramedic|therapist|psychologist|\b(?:dr|em[st])\b)/i),
