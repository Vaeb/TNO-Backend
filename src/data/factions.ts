...
export const npFactionsRegex = {
    ...
    police: noLater(noFormer(
        mergeRegex([
            ...
            /(?:\bcop\b|chief of police|officer|der?puty(?![\s\-_]*(?:mayor|warden))|\bd-\d|\bdispatch\b|ride[\s\-_.]*along|sergeant|lieutenant|corporal|sheriff|\btrooper|cadet|\b(?:ranger|dt|sgt|cpl|cpt|ofc|lspd|sasp|bcso|police[\s\-_]*academy|lt(?![^|!]*\bjones\b))\b)/i,
            /(?!\?|[\s\-_]+copium)/i,
        ])
    )),
